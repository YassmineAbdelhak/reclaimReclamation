...
const Reclamations = props => (
  ...
  <FormGroup row>
    <Label sm={2}><strong> Responsible:</strong></Label>
      <Col sm={10}>{props.employee}</Col>
  </FormGroup> 
  <FormGroup row>
    <Label sm={10}></Label>
    <Button color="primary"><Link to={"/updateRecW/"+props.id}><FontAwesomeIcon icon={faEdit} color="white" /> </Link> </Button>
  </FormGroup>
</Alert>
)
export default class AllReclamation extends Component {
  ...
  reclamationList() {
    return this.state.forms.map(currentrec => {
      if((currentrec.position == "Working On") && (currentrec.departement._id == this.state.departement) && (currentrec.employee._id == this.state.id) ){
